...
public class ConnectXController {
    ...
    public ConnectXController(IGameBoard model, ConnectXView view, int np) {
        ...
        count = 0;
        playerToken = new char[MAX_PLAYERS];
        playerToken[0] = 'X';
        playerToken[1] = 'O';
        playerToken[2] = 'Y';
        playerToken[3] = 'P';
    ...
    public void processButtonClick(int col) {
        for(int i = 0; i < curGame.getNumColumns(); ++i){
            ...
            if(curGame.checkForWin(i) && curGame.whatsAtPos(pos) != ' '){
                newGame();
                return;
            }
        }
        ...
        if(!curGame.checkIfFree(col)){
            ...
            for(int i = 0; i < curGame.getNumRows(); ++i){
                ...
                if(curGame.whatsAtPos(pos) == ' '){
                    screen.setMarker(i, col, playerToken[count % numPlayers]);
                    break;
                }
            }
            curGame.placeToken(playerToken[count % numPlayers], col);
            if(curGame.checkForWin(col)){
                screen.setMessage("Player " + playerToken[count % numPlayers] + " has won!");
                count++;
            }
            else if(curGame.checkTie()){
                screen.setMessage("Tie!");
                count++;
            }else{
                count++;
                screen.setMessage("It is " + playerToken[count % numPlayers] +"\'s turn");
            }
